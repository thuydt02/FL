...
class MNIST_truncated(data.Dataset):
    ...
    def __build_truncated_dataset__(self):
        ...
        #if MLP => no reshape needed
        return data.reshape(num_samples, 1, 28, 28), target
...
class CIFAR10_truncated(data.Dataset):
    ...
    def __build_truncated_dataset__(self):
        ...
        m = torch.max(data) * 1.0
        data = data / m

        #permute(0, 3, 1, 2) = keeping the first (0) dim, the seond dim is from the 4 (3) dim, ...
        #because the shape of cnn is [batch_size, num_channels, img_height, img_width] 
        #while the dataset gives us [num_sample, img_height, img_width, num_channels]

        return data.permute(0, 3, 1,2), torch.tensor(target, dtype=torch.long) 
...
class FEMNIST(MNIST):
    ...
    def __init__(self, root, dataidxs=None, train=True, transform=None, target_transform=None,
        ...
        self.data, self.target, self.users_index = torch.load(os.path.join(self.processed_folder, data_file))
